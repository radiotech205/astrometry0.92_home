...
void printHelp_checktree(char* progname) {
    printf("\nUsage: %s\n"
           "     [-d]: print data\n"
...
const char* OPTIONS_checktree = "hdn";

int main_checktree(int argc, char** args) {
    int argchar;
    ...
    while ((argchar = getopt(argc, args, OPTIONS_checktree)) != -1)
        switch (argchar) {
        case 'h':
            printHelp_checktree(progname);
            exit(-1);
        case 'd':
            printData = 1;
            break;
