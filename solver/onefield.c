...
static resize_field_xy(starxy_t** fieldxy_file, int num_stars);
...
static void solve_fields(onefield_t* bp, sip_t* verify_wcs) {
    ...
    for (fi = 0; fi < il_size(bp->fieldlist); fi++) {
        ...
        // Get the field.
        starxy_t* fieldxy_file = xylist_read_field(bp->xyls, NULL);
        // FIXME: My function
        resize_field_xy(&fieldxy_file, NUM_STARS);

        solver_set_field(sp, fieldxy_file);
        if (!sp->fieldxy_orig) {
...
static resize_field_xy(starxy_t** fieldxy_file, int num_stars) {
    int n = starxy_n(*fieldxy_file);
    if(n > num_stars) {
        starxy_t* fieldxy_temp = starxy_new(num_stars, 0, 0);
        for(int i = 0; i < num_stars; i++) {
            starxy_setx(fieldxy_temp, i, starxy_getx(*fieldxy_file, i));
            starxy_sety(fieldxy_temp, i, starxy_gety(*fieldxy_file, i));
        }
        starxy_free_data(*fieldxy_file);
        *fieldxy_file = starxy_new(num_stars, 0, 0);
        for(int i = 0; i < num_stars; i++) {
            starxy_setx(*fieldxy_file, i, starxy_getx(fieldxy_temp, i));
            starxy_sety(*fieldxy_file, i, starxy_gety(fieldxy_temp, i));
        }
        starxy_free(fieldxy_temp);
    }
}
