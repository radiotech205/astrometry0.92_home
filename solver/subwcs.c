...
const char* OPTIONS_subwcs = "hve:x:y:X:Y:s:";

void printHelp_subwcs(char* progname) {
...
int main_subwcs(int argc, char** args) {
    int argchar;
    int loglvl = LOG_MSG;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_subwcs)) != -1) {
        switch (argchar) {
        case 'v':
            loglvl++;
            break;
