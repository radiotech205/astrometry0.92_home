...
const char* OPTIONS_uniformize_catalog = "hvH:s:n:N:d:R:D:S:fm:";

void printHelp_uniformize_catalog(char* progname) {
...
int main_uniformize_catalog(int argc, char *argv[]) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_uniformize_catalog)) != -1)
        switch (argchar) {
        case 'R':
            racol = optarg;
            break;
        ...
        case 'h':
            printHelp_uniformize_catalog(progname);
            return 0;
        default:
            return -1;
        }
