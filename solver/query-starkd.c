...
static const char* OPTIONS_query_starkd = "hvr:d:R:t:Io:T";

void printHelp_query_starkd(char* progname) {
...
int main_query_starkd(int argc, char **argv) {
    int argchar;
    startree_t* starkd;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_query_starkd)) != -1)
        switch (argchar) {
        case 'o':
            rdfn = optarg;
            break;
