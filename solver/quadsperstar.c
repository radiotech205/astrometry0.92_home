...
void print_help_quadsperstar(char* progname)
{
...
int main_quadsperstar(int argc, char** args) {
    int argchar;
    char* basefn;
    qidxfile* qf;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_quadsperstar)) != -1)
        switch (argchar) {
        case 'h':
            print_help_quadsperstar(args[0]);
            exit(0);
        }
