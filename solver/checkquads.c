...
#define OPTIONS_checkquads "hf:"
const char HelpString_checkquads[] =
    "quadidx -f fname\n";


int main_checkquads(int argc, char *argv[]) {
    int argidx, argchar;
    char *qidxfname = NULL;
    char *quadfname = NULL;
    quadfile_t* quad;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_checkquads)) != -1)
        switch (argchar) {
        case 'f':
        ...
        case 'h':
            fprintf(stderr, HelpString_checkquads);
            return (HELP_ERR);
        default:
            return (OPT_ERR);
        }
    ...
    quad = quadfile_open(quadfname);
    if (!quad) {
        fprintf(stderr, "Couldn't open quads file %s.\n", quadfname);
        exit(-1);
    }

    qidx = qidxfile_open(qidxfname);
    if (!qidx) {
