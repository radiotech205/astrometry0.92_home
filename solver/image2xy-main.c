...
static const char* OPTIONS_image2xy = "hi:Oo:8Hd:D:ve:B:S:M:s:p:P:bU:g:C:m:a:G:w:L:";

static void printHelp_image2xy() {
    fprintf(stderr,
...
int main_image2xy(int argc, char *argv[]) {
    int argchar;
    char* outfn = NULL;
    char* infn;
    int overwrite = 0;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_image2xy)) != -1) {
        switch (argchar) {
        case 'L':
