...
const char* OPTIONS_printsolved = "hum:SM:jwps";

void printHelp_printsolved(char* progname) {
...
int main_printsolved(int argc, char** args) {
    int argchar;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_printsolved)) != -1) {
        switch (argchar) {
        case 's':
            summary = TRUE;
            break;
        ...
        case 'h':
        default:
            printHelp_printsolved(progname);
            exit(-1);
        }
    }
    if (optind < argc) {
