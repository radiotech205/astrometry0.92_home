...
const char* OPTIONS_create_scamp_catalog = "hv";

void print_help_create_scamp_catalog(char* progname) {
...
int main_create_scamp_catalog(int argc, char** args) {
    int c;
    char* infn = NULL;
    char* outfn = NULL;
    scamp_cat_t* scamp;
    ...
    while ((c = getopt(argc, args, OPTIONS_create_scamp_catalog)) != -1) {
        switch (c) {
        case 'h':
            print_help_create_scamp_catalog(args[0]);
            exit(0);
        case 'v':
            loglvl++;
            break;
