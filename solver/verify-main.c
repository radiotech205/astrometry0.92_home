...
#include "memory.h"
static const char* OPTIONS_verify = "hx:w:r:vj:p:";

void print_help_verify(char* progname) {
...
int main_verify(int argc, char** args) {
    ...
    int loglvl = /*LOG_MSG*/LOG_ALL;
    double wcsscale;

    fits_use_error_system();

    while ((c = getopt(argc, args, OPTIONS_verify)) != -1) {
        switch (c) {
        case 'p':
            plotfn = optarg;
            break;
