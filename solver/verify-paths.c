...
static const char* OPTIONS_verify_paths = "hvi:m:f:r:p";

static void print_help_verify_paths(const char* progname) {
    printf("Usage:   %s\n"
           "   -m <match-file>\n"
           "   -f <xylist-file>\n"
           "  (    -i <index-file>\n"
...
int main_verify_paths(int argc, char** args) {
    int argchar;
    int loglvl = LOG_MSG;
    char* indexfn = NULL;
    char* matchfn = NULL;
    ...
    while ((argchar = getopt(argc, args, OPTIONS_verify_paths)) != -1)
        switch (argchar) {
        case 'p':
            do_paths = TRUE;
            break;
        ...
        case 'm':
            ...
            break;
        case 'h':
            print_help_verify_paths(args[0]);
            exit(0);
        case 'v':
            loglvl++;
            break;
    ...
    if (indexfn) {
        index = index_load(indexfn, 0, index);
        if (!index) {
