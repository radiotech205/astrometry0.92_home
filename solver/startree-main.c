...
const char* OPTIONS_startree = "hvL:d:t:bsSci:o:R:D:PTkn:";

void printHelp_startree(char* progname) {
...
int main_startree(int argc, char *argv[]) {
    int argidx, argchar;
    startree_t* starkd;
    fitstable_t* cat;
    fitstable_t* tag;
    ...
    if (argc <= 2) {
        printHelp_startree(progname);
        return 0;
    }

    while ((argchar = getopt (argc, argv, OPTIONS_startree)) != -1)
        switch (argchar) {
        case 'T':
            tagalong_first = TRUE;
            break;
