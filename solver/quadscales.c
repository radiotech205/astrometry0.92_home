...
#define OPTIONS_quadidx "h"


void print_help_quadidx(char* progname)
{
...
int main_quadscales(int argc, char** args) {
    int argchar;
    char* basename;
    char* fn;
    quadfile_t* qf;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_quadidx)) != -1)
        switch (argchar) {
        case 'n':
            Nbins = atoi(optarg);
            break;
        case 'h':
            print_help_quadidx(args[0]);
            exit(0);
        }
