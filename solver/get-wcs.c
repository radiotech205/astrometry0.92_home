...
static char* OPTIONS_get_wcs = "ho:e:t";

static void printHelp_get_wcs(char* progname) {
    printf("%s <input-file>\n"
           "   [-e <extension>]\n"
           "   [-o <output-file>]\n"
           "   [-t]: force TAN (not SIP)\n"
...
int main_get_wcs(int argc, char *argv[]) {
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_get_wcs)) != -1)
        switch (argchar) {
        case '?':
        case 'h':
            printHelp_get_wcs(progname);
            return 0;
        case 'e':
            ext = atoi(optarg);
            break;
