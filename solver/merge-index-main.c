...
#define OPTIONS_merge_index "hq:c:s:o:"

static void printHelp_merge_index(char* progname) {
...
int main_merge_index(int argc, char **args) {
    int argchar;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_merge_index)) != -1)
        switch (argchar) {
        case 'q':
            quadfn = optarg;
            break;
        ...
        case 'h':
            printHelp_merge_index(progname);
            return 0;
        default:
            return -1;
        }
