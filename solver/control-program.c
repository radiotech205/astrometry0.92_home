...
static const char* OPTIONS_control_program = "hvc:a:A:W:H:I";


static void print_help_control_program(const char* progname) {
    printf("Usage:   %s\n"
...
int main_control_program(int argc, char** args) {
    ...
    while ((c = getopt(argc, args, OPTIONS_control_program)) != -1)
        switch (c) {
        case 'h':
            print_help_control_program(args[0]);
            exit(0);
        case 'v':
            loglvl++;
            break;
