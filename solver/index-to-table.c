...
static const char* OPTIONS_index_to_table = "hi:o:";

void printHelp_index_to_table(char* progname) {
...
int main_index_to_table(int argc, char *argv[]) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_index_to_table)) != -1) {
        switch (argchar) {
        case 'i':
            indexfn = optarg;
            break;
        ...
        default:
        case 'h':
            printHelp_index_to_table(progname);
            exit(0);
        }
    }
