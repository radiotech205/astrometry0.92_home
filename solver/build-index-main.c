...
const char* OPTIONS_build_index = "hvi:o:N:l:u:S:fU:H:s:m:n:r:d:p:R:L:EI:MTj:1:P:B:A:D:Kt:e:";

static void print_help_build_index(char* progname) {
...
int main_build_index(int argc, char** argv) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_build_index)) != -1)
        switch (argchar) {
        case 'A':
            p->racol = optarg;
            break;
