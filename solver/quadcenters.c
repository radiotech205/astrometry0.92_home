...
#define OPTIONS_quadcenters "hr:R"


void print_help_quadcenters(char* progname)
{
...
int main_quadcenters(int argc, char** args) {
    int argchar;
    char* basename;
    char* outfn = NULL;
    index_t* index;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_quadcenters)) != -1)
        switch (argchar) {
        case 'R':
            addradius = TRUE;
            break;
