...
const char* OPTIONS_local_index = "hvx:w:l:u:o:d:I:N:p:R:L:Mn:U:S:f:r:J:X:Y:";

static void print_help_local_index(char* progname) {
...
int main_local_index(int argc, char** argv) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_local_index)) != -1)
        switch (argchar) {
        case 'X':
            xcol = optarg;
            break;
