...
char* OPTIONS_rdlstohealpix = "hqf:N:";

void printHelp_rdlstohealpix(char* progname) {
...
int main_rdlstohealpix(int argc, char** args) {
    char* filename = NULL;
    int npoints;
    int i, j;
    int* healpixes;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_rdlstohealpix)) != -1)
        switch (argchar) {
        case 'N':
            Nside = atoi(optarg);
            break;
        case 'f':
            ...
            break;
        case 'h':
            printHelp_rdlstohealpix(progname);
            exit(0);
        case 'q':
            quiet = TRUE;
            break;
