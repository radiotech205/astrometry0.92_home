...
const char* OPTIONS_wcs_grab = "hv";

static void print_help_wcs_grab(char* progname) {
    BOILERPLATE_HELP_HEADER(stdout);
    printf("\nUsage:  %s <input-filename> <extension> <output-filename>\n"
           "\n\n", progname);
}

int main_wcs_grab(int argc, char** args) {
    char* outfn = NULL;
    char* infn = NULL;
    int ext;
    int c;
    ...
    while ((c = getopt(argc, args, OPTIONS_wcs_grab)) != -1) {
        switch (c) {
        case '?':
        case 'h':
            print_help_wcs_grab(args[0]);
            exit(0);
        case 'v':
            loglvl++;
            break;
