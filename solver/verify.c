...
//    theta = usage_memory(v->NT * sizeof(int), &result);
    ...
    logodds = 0.0;
    mu = 0;
    for (i=0; i<v->NT; i++) { // проход по тестовым звёздам.
        const double* testxy;
        double sig2;
        int refi;
        int tmpi;
...
void verify_hit(const startree_t* skdt, int index_cutnside, MatchObj* mo,
    ...
    С опорными звездами нужно быть немного осторожнее:

    Мы хотим иметь возможность возвращать список всех опорных
    звезд на изображении, но в процессе проверки мы
    хотим применить некоторую фильтрацию опорных звезд.
    Поэтому мы
    храним целочисленный массив ("refperm") индексов в
    массивах количества опорных звезд. Есть "NR" хороших
    звезд, но "NRall" в общей сложности. Таким образом, операции над всеми
    звездами должны переходить в "NRall" в исходных массивах, но
    операции над хорошими звездами должны переходить в "NR", используя "refperm" для
    перенаправления.

    Это означает, что "refperm" должен оставаться массивом перестановок (т.е.,
    без дубликатов), и каждое значение должно быть меньше "NRall"; при
    фильтрации индекса он должен перемещаться в часть
    массива между "NR" и "NRall". Мы используем массив "badguys" для
    временного хранения этих индексов.
     */
    assert(skdt->sweep);
    // Find all index stars within the bounding circle of the field.
