...
static const char* OPTIONS_index_stats = "hvmM:";


void print_help_index_stats(char* progname) {
...
int main_index_stats(int argc, char** args) {
    int argchar;
    anbool measmags = FALSE;
    char* magcol = "mag";
    int loglvl = LOG_MSG;

    while ((argchar = getopt (argc, args, OPTIONS_index_stats)) != -1)
        switch (argchar) {
        case 'm':
            measmags = TRUE;
            break;
