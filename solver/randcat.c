...
#define OPTIONS_randcat "hn:f:r:R:d:D:S:"
const char HelpString_randcat[] =
...
int RANDSEED = 999;
int DEFAULT_RAMIN = 0;
int DEFAULT_RAMAX = 360;
int DEFAULT_DECMIN = -90;
int DEFAULT_DECMAX = 90;

int main_randcat(int argc, char *argv[])
{
    int argidx, argchar;
    int numstars = 10;
    char *fname = NULL;
    ...
    if (argc <= 4) {
        fprintf(stderr, HelpString_randcat);
        return (OPT_ERR);
    }

    while ((argchar = getopt (argc, argv, OPTIONS_randcat)) != -1)
        switch (argchar) {
        case 'S':
