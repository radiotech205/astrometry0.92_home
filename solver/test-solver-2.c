...
void testit(int* wanted, int Nwanted, int dimquads, int (*compar)(const void *, const void *),
    ...
    starxy = field1();
    quadlist_ts = bl_new(16, dimquads*sizeof(int));
    ninv = 0;
    solver = solver_new();
    ...
    printf("\n");
    printf("N found: %i; N wanted: %i\n", bl_size(quadlist_ts), Nwanted);
    printf("N obeying invariants: %i\n", ninv);
    assert(bl_size(quadlist_ts) == Nwanted);
    for (i=0; i<bl_size(quadlist_ts); i++) {
        //int* i1 = bl_access(quadlist, i);
        //int* i2 = wanted[i];
        //printf("[%i, %i, %i] vs [%i, %i, %i]\n", i1[0],i1[1],i1[2], i2[0],i2[1],i2[2]);
        assert(compar(bl_access(quadlist_ts, i), wanted+i*dimquads) == 0);
    }
    bl_free(quadlist_ts);
}
...
char* OPTIONS_test_solver2 = "v";

int main_test_solver2(int argc, char** args) {
    int argchar;
    int* flatwanted;
    int Nwanted;
    int itemsize;
    ...
    while ((argchar = getopt(argc, args, OPTIONS_test_solver2)) != -1)
        switch (argchar) {
        case 'v':
            log_init(LOG_ALL+1);
            break;
