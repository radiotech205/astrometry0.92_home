...
static const char* OPTIONS_index_info = "hvr:d:R:";

void printHelp_index_info(char* progname) {
...
int main_index_info(int argc, char **argv) {
    int argchar;
    int loglvl = LOG_MSG;
    char** myargs;
    int nmyargs;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_index_info)) != -1)
        switch (argchar) {
        case 'v':
            loglvl++;
            break;
