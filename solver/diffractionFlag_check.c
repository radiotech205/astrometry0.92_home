...
#define OPTIONS_diffractionFlag_check "h"

void print_help_diffractionFlag_check(char* progname) {
...
int main_diffractionFlag_check(int argc, char** args) {
    int c;
    int i;
    int fnum = 0, fnum1 = 0, d = 10;


    while ((c = getopt(argc, args, OPTIONS_diffractionFlag_check)) != -1) {
        switch (c) {
        case '?':
        case 'h':
            print_help_diffractionFlag_check(args[0]);
            exit(0);
        }
    }
