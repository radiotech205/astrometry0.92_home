...
void solver_compute_quad_range(const solver_t* sp, const index_t* index,
    ...
    // Вычислить поправочный коэффициент для масштаба четырехугольника: каковы крайние
    // диапазоны масштабов четырехугольников, которые должны быть допустимы, учитывая
    // допуск кода?
    // -Каково максимальное количество пикселей, на которое может переместиться звезда C или D,
    // чтобы в одиночку превысить допуск кода?
    // -Наибольший четырехугольник
    // -Наименьшее масштабирование в угловых секундах на пиксель
    // -index_scale_upper * 1/sqrt(2) - это длина стороны
    // единичного квадрата кодового пространства в угловых секундах.
    // -Это значение, умноженное на допуск кода, показывает, насколько далеко звезда C/D
    // может переместиться, прежде чем превысить допуск кода, в угловых секундах.
    // -Это значение, разделенное на наименьшее масштабирование в угловых секундах на пиксель,
    // дает наибольшее перемещение в пикселях.
...
static void check_inbox(pquad* pq, int start, solver_t* solver) {
    ...
    for (i = start; i < pq->ninbox; i++) {
        ...
        setx(pq->xy, i, Cx); // тут хранится хеш
        sety(pq->xy, i, Cy); // тут хранится хеш
    }
}
...
void solver_preprocess_field(solver_t* solver) {
    ...
    // FIXME: My edit
//    int num_stars = starxy_n(solver->fieldxy_orig);
//    if(num_stars > NUM_STARS)   num_stars = NUM_STARS;
//    solver->fieldxy = starxy_new(num_stars, 0, 0);
//    for(int i = 0; i < num_stars; i++) {
//        starxy_setx(solver->fieldxy, i, starxy_getx(solver->fieldxy_orig, i));
//        starxy_sety(solver->fieldxy, i, starxy_gety(solver->fieldxy_orig, i));
//    }
...
Довольно сложная рекурсивная функция: звезды A и B уже выбраны,
поэтому система координат кода зафиксирована, и мы
уже определили, какие другие звезды создадут допустимые коды (т.е. находятся
в "ящик"). Теперь мы хотим построить признаки, используя все наборы допустимых
звезд (без перестановок).

pq - данные, связанные с парой AB.
field - массив номеров звезд поля.
fieldoffset - смещение в массиве поля, куда следует добавить первую звезду.
n_to_add - количество добавляемых звезд.
adding - звезда, которую мы добавляем в данный момент (в диапазоне [0, n_to_add).
fieldtop - максимальное количество звезд поля, из которых можно построить квады.
dimquad, solver, tol2 - передаются в try_all_codes.
...
void solver_run(solver_t* solver) {
    ...
    double tol2;
    int field[DQMAX];
    // что - то с коллбеком на время.
    get_resource_stats(&usertime, &systime, NULL);
    // опц. заполнение даннми звёзд.
    if (!solver->vf)
        solver_preprocess_field(solver);
    ...
    {   // вычисление пределов значений для размеров квада для данного индекса.
        ...
        for (i = 0; i < num_indexes; i++) {
            ...
            // Ограничения на размер четырехугольников, которые мы пытаемся сопоставить, в пикселях.
            // Получено из index_scale_* и funits_*.
         ...
         */
        // Выделение места под потенциальные квады.
        /* каждая структура соответствует одному выбору звезд A и B; структура
         по индексу (B * numxy + A) содержит информацию о квадах, которые могут быть
         созданы с использованием звезд A, B.
         Для каждой пары AB мы кэшируем параметры масштаба и вращения,
         и храним массив «inbox» длиной «numxy» логических значений, по одному для
...
//                    pq->xy = usage_memory(numxy * 2 * sizeof(double), &result);
                    ...
                    memset(pq->inbox, TRUE, solver->startobj * sizeof(anbool));
         ...
         */
        /*Каждый раз, проходя цикл "for" ниже, мы рассматриваем новую звезду
        ("newpoint"). Сначала мы пытаемся построить все четырехугольники, у которых новая
        звезда находится на диагонали (звезда B).
        Затем мы пытаемся построить все четырехугольники, у которых
        звезда не находится на диагонали (звезда D).
        Для каждой пары AB у нас есть структура "potential_quad" или "pquad".
        В ней кэшируются необходимые вычисления: решение о том, является ли
        масштаб приемлемым, вычисление преобразования в кодовые
        координаты и решение о том, какие звезды C и D находятся в круге.*/
        for (newpoint = solver->startobj; newpoint < numxy; newpoint++) {
            ...
            for (field[A] = 0; field[A] < newpoint; field[A]++) {
                ...
                memset(pq->inbox, TRUE, (newpoint + 1) * sizeof(anbool));
                pq->ninbox = newpoint + 1;
                // -except A and B.
                pq->inbox[field[A]] = FALSE;
                pq->inbox[field[B]] = FALSE;
                check_inbox(pq, 0, solver); // тут в pq->xy хранится хеш, в соответствии с pq->inbox == TRUE.
                debug("    inbox(A=%i, B=%i): ", field[A], field[B]);
                print_inbox(pq);
            }
            ...
            for (field[A] = 0; field[A] < newpoint; field[A]++) {
                for (field[B] = field[A] + 1; field[B] < newpoint; field[B]++) {
                    ...
                    pq->ninbox = field[C] + 1;
                    if((field[A] > newpoint) || (field[B] > newpoint) || (field[C] > newpoint))
                        printf("(field[X] > newpoint)\n");
                    if(pq->ninbox > numxy)
                         printf("(pq->ninbox > numxy)\n");
...
static void try_permutations(const int* origstars, int dimquad,
    ...
    Это рекурсивная функция, которая перебирает все комбинации

    внутренних звезд (т. е. не звезд A, B, которые образуют "основу"
    квадрата).

    Мы заполняем массив "stars" идентификаторами звезд (из "origstars")
    звезд, образующих квад, одновременно заполняя массив "code" соответствующими кодовыми координатами (из
    "origcode").

    Например, если "dimquad" равно 5, а "origstars" содержит
    A,B,C,D,E, мы хотим вызвать функцию "resolve_matches" со следующими
    комбинациями в "stars":
    ...
    Этот вызов попытается поместить каждую звезду в "slot" по очереди, а затем для каждой из них рекурсивно перейдет к "slot" в остальных звездах.

    Обратите внимание, что мы заполняем звезды[2], звезды[3] и т. д.; первые два
    элемента уже заполнены звездами A и B.
...
static int solver_handle_hit(solver_t* sp, MatchObj* mo, sip_t* verifysip,
    ...
    if (sp->predistort || (sp->pixel_xscale > 0)) {
        ...
        N = mo->nbest;
        int result;
        //matchxy = usage_memory(N * 2 * sizeof(double), &result);
        matchxy = usage_memory2(N * 2 * sizeof(double), &result, solver_handle_hit);
        if(!matchxy || result)  return -1;

        //matchxyz = usage_memory(N * 3 * sizeof(double), &result);
        matchxyz = usage_memory2(N * 3 * sizeof(double), &result, solver_handle_hit);
        if(!matchxyz || result)  return -1;

        //weights = usage_memory(N * sizeof(double), &result);
        weights = usage_memory2(N * sizeof(double), &result, solver_handle_hit);
        if(!weights || result)  return -1;
