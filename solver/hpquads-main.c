...
static const char* OPTIONS_hpquads = "hi:c:q:bn:u:l:d:p:r:L:RI:F:HEv";

static void print_help_hpquads(char* progname) {
...
int main_hpquads(int argc, char** argv) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_hpquads)) != -1)
        switch (argchar) {
        case 'v':
            loglvl++;
            break;
