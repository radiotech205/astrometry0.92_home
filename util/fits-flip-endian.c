...
char* OPTIONS_fits_flip_endian = "hi:o:e:s:";

void printHelp_fits_flip_endian(char* progname) {
...
int main_fits_flip_endian(int argc, char *argv[]) {
    int argchar;
    char* infn = NULL;
    char* outfn = NULL;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_fits_flip_endian)) != -1)
        switch (argchar) {
        case 'e':
        ...
        case '?':
        case 'h':
            printHelp_fits_flip_endian(progname);
            return 0;
        default:
            return -1;
        }
