...
const char* OPTIONS_wcs_pv2sip = "hve:sx:X:y:Y:a:W:H:to:S";

void print_help_wcs_pv2sip(char* progname) {
...
int main_wcs_pv2sip(int argc, char** args) {
    int loglvl = LOG_MSG;
    char** myargs;
    int nargs;
    int c;
    ...
    while ((c = getopt(argc, args, OPTIONS_wcs_pv2sip)) != -1) {
        switch (c) {
        case 'S':
            doshift = 0;
            break;
