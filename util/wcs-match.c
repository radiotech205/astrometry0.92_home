...
const char* OPTIONS_wcs_match = "vh1:2:";
static int loglvl = LOG_MSG;

static anwcs_t* wcs1 = NULL;
static anwcs_t* wcs2 = NULL;

void print_help_wcs_match(char* progname) {
...
int main_wcs_match(int argc, char** args) {
    int ext = 0,c;
    double ra,dec;
    double sol[2];
    ...
    while ((c = getopt(argc, args, OPTIONS_wcs_match)) != -1) {
        switch(c) {
        case 'v':
            loglvl++;
            break;
        case 'h':
            print_help_wcs_match(args[0]);
            exit(0);
        case '1':
            wcsfn1 = optarg;
            break;
