...
const char* OPTIONS_hpsplit = "hvn:r:d:m:o:gc:e:t:b:RC";

void printHelp_hpsplit(char* progname) {
...
int main_hpsplit(int argc, char *argv[]) {
    int argchar;
    ...
    while ((argchar = getopt (argc, argv, OPTIONS_hpsplit)) != -1)
        switch (argchar) {
        case 'C':
            closefiles = TRUE;
            break;
