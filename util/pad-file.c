...
const char* OPTIONS_pad_file = "hv:c:";

void printHelp_pad_file(char* progname) {
...
int main_pad_file(int argc, char** args) {
    int argchar;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_pad_file)) != -1) {
        switch (argchar) {
        case 'v':
            padchar = atoi(optarg);
