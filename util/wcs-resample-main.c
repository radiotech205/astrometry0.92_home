...
const char* OPTIONS_wcs_resample = "hw:e:E:x:L:z";

void print_help_wcs_resample(char* progname) {
...
int main_wcs_resample(int argc, char** args) {
    int c;
    char* inwcsfn = NULL;
    char* outwcsfn = NULL;
    char* infitsfn = NULL;
    ...
    while ((c = getopt(argc, args, OPTIONS_wcs_resample)) != -1) {
        switch (c) {
        case 'h':
            print_help_wcs_resample(args[0]);
            exit(0);
        case 'w':
            inwcsfn = optarg;
            break;
