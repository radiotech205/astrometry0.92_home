...
static const char* OPTIONS_coadd = "hvw:o:e:O:Ns:p:D";

void printHelp_coadd(char* progname) {
...
int main_coadd(int argc, char** args) {
    int argchar;
    ...
    while ((argchar = getopt(argc, args, OPTIONS_coadd)) != -1)
        switch (argchar) {
        case '?':
        case 'h':
            printHelp_coadd(progname);
            exit(0);
        case 'D':
            divweight = TRUE;
            break;
