...
#define OPTIONS_get_healpix "hdN:npH:drR:"

void print_help_get_healpix(char* progname) {
    printf("usage:\n"
           "  %s [options] ra dec\n"
...
int main_get_healpix(int argc, char** args) {
    int healpix = -1;
    int i;
    anbool degrees = TRUE;
    ...
    if (argc == 1) {
        print_help_get_healpix(args[0]);
        exit(0);
    }

    while ((c = getopt(argc, args, OPTIONS_get_healpix)) != -1) {
        switch (c) {
        case '?':
        case 'h':
            print_help_get_healpix(args[0]);
