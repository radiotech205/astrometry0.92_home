...
static const char* OPTIONS_an_pnmtofits = "hvqo:";

static void printHelp_an_pnmtofits(char* progname) {
...
int main_an_pnmtofits(int argc, char** args) {
    int argchar;
    char* infn = NULL;
    char* outfn = NULL;
    unsigned int row;
    ...
    while ((argchar = getopt (argc, args, OPTIONS_an_pnmtofits)) != -1) {
        switch (argchar) {
        case '?':
        case 'h':
            printHelp_an_pnmtofits(progname);
            exit(0);
        case 'v':
            loglvl++;
            break;
